...
Three questions are open and deliberately not guessed at.

1. **The clause-vote ceiling.** Does a clause's vote max out at `LF`, or at `min(n_literals, LF)`?
   This is the *unit* of the vote. It sets whether short general clauses vote as loudly as long
   specific ones, whether `L` and `LF` are independent knobs, how `T` is calibrated, and whether
   votes are comparable across clauses at all. It blocks the evaluator, and it fails silently rather
   than loudly if guessed wrong.
2. **Bit- versus symbol-granular fuzziness.** Over thermometer or bag-of-words features a partial
...
3. **Deliverable form.** One package or several, and whether the model format is pitched for
...
question 3, and this layout does not pre-empt it.
...
| Track | What | State |
|---|---|---|
| A | Substrate — evaluator, typed feedback, model format, harness | blocked on open questions 1-2 |
...
upstream Tsetlin.jl and a published model, so it returns an answer before any of the open questions
above are settled. It is falsifiable in both directions, which is the point.
